...
firstRepeat ::
  ...
  List a
  -> Optional a
firstRepeat xs = eval (go xs) S.empty
  where
    go :: (Ord a) => List a -> State (S.Set a) (Optional a)
    go Nil = pure Empty
    go (a:.as) = get >>= (\set -> if S.member a set
                                  then return (Full a)
                                  else put (S.insert a set) >> go as)
